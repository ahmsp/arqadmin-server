...
You can access your application via **`localhost`**, if you're running the containers directly, or through **`192.168.33.235`** when run on a vm. nginx and mailhog both respond to any hostname, in case you want to add your own hostname on your `/etc/hosts` 

Service|Address outside containers|Address outside VM
------|---------|-----------
Webserver|[localhost:8990](http://localhost:8990)|[192.168.33.235](http://192.168.33.235)
...
------|---------|-----------
php-fpm|arqadmin-php-fpm|9000
MySQL|arqadmin-mysql|3306 (default)
Redis|arqadmin-redis|6379 (default)
...
**Note 2:** if you're using Vagrant, you'll need to ssh into it first
  ...
  * Execute command inside of container: `docker exec -it arqadmin-php-fpm COMMAND` where `COMMAND` is whatever you want to run. For instance, `/bin/bash` to open a console prompt.
